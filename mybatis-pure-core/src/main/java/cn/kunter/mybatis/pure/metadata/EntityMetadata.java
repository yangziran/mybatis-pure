...
public final class EntityMetadata {
	...
	public AliasableSqlTable<?> table(String alias) {
		throw new UnsupportedOperationException(
				"Table aliasing is delegated to the specific AliasableSqlTable implementation.");
	}
	...
	public ColumnMetadata column(String property) {
		...
		if (col == null) {
			throw new IllegalArgumentException(
					"Property '" + property + "' not found or excluded in entity " + entityType.getName());
		}
		return col;
	}
	...
	public BasicColumn[] selectColumns() {
		return propertyColumnMap.values()
			.stream()
			.map(col -> col.column().as(col.property()))
