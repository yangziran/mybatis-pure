...
public interface BaseMapperPure<T> {
	...
	/** @apiNote 内部方法，执行计数查询 */
	@SelectProvider(type = SqlProviderAdapter.class, method = "select")
	long __executeCount(org.mybatis.dynamic.sql.select.render.SelectStatementProvider statement);
	...
	default List<AndOrCriteriaGroup> getGlobalFilters() {
		...
		metadata().logicDeleteColumn().ifPresent(col -> {
			...
			if (logicUnDeletedValue != null) {
				criteria
					.add(SqlBuilder.and((SqlColumn<Object>) col.column(), SqlBuilder.isEqualTo(logicUnDeletedValue)));
			}
		});
	...
	default Optional<T> selectById(Object id) {
		SqlColumn<Object> idCol = (SqlColumn<Object>) metadata().idColumn()
			.orElseThrow(
					() -> new IllegalArgumentException("No @TableId found for " + metadata().entityType().getName()))
			.column();
	...
	default int insert(T entity) {
		...
		return MyBatis3Utils.insert(this::__executeInsert, entity, table(), c -> c);
	}
	...
	default int insertBatch(Collection<? extends T> entities) {
		...
		@SuppressWarnings("unchecked")
		Collection<T> castedEntities = (Collection<T>) entities;
		return MyBatis3Utils.insertMultiple(this::__executeInsertMultiple, castedEntities, table(), c -> c);
	}
	 ...
	 */
	default int update(
			Consumer<org.mybatis.dynamic.sql.update.UpdateDSL<org.mybatis.dynamic.sql.update.UpdateModel>> applier) {
		var builder = SqlBuilder.update(table());
		if (applier != null)
	 ...
	 * @throws IllegalStateException 如果没有 WHERE 条件
	 */
	default int delete(org.mybatis.dynamic.sql.where.WhereApplier whereApplier) {
		if (metadata().logicDeleteColumn().isPresent()) {
			var colMeta = metadata().logicDeleteColumn().get();
			var builder = SqlBuilder.update(table());
			builder.set((SqlColumn<Object>) colMeta.column()).equalTo(colMeta.logicDeletedValue());
	 ...
	 * @return 影响行数
	 */
	default int deleteAll(org.mybatis.dynamic.sql.where.WhereApplier whereApplier) {
		if (metadata().logicDeleteColumn().isPresent()) {
			var colMeta = metadata().logicDeleteColumn().get();
			var builder = SqlBuilder.update(table());
			builder.set((SqlColumn<Object>) colMeta.column()).equalTo(colMeta.logicDeletedValue());
