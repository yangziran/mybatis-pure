...
import cn.kunter.mybatis.pure.annotation.FieldFill;
import cn.kunter.mybatis.pure.annotation.LogicDelete;
import cn.kunter.mybatis.pure.annotation.Table;
import cn.kunter.mybatis.pure.annotation.TableField;
...
public final class MetadataCache {
	...
	private static EntityMetadata resolve(Class<?> type) {
		...
		String tableName = (tableAnnotation != null && !tableAnnotation.value().isEmpty()) ? tableAnnotation.value()
		...
		for (Class<?> current = type; current != null && current != Object.class; current = current.getSuperclass()) {
			for (Field field : current.getDeclaredFields()) {
				if (metadata.hasColumn(field.getName()))
					continue;
				...
				else if (idAnnotation != null && !idAnnotation.value().isEmpty()) {
