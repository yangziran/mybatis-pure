...
public class SupportClassGenerator {
	...
	public void generate(TypeElement typeElement, List<VariableElement> fields) {
		...
		MethodSpec constructor = MethodSpec.constructorBuilder()
			...
			.addStatement("super($S, $T::new)", tableName, tableClassName)
			.build();
		...
		for (VariableElement field : fields) {
			...
			FieldSpec columnField = FieldSpec
				.builder(ParameterizedTypeName.get(sqlColumnClass, fieldType), fieldName, Modifier.PUBLIC,
						Modifier.FINAL)
				.initializer("column($S, $T.$L)", columnName, JDBCType.class, jdbcType)
				.build();
		...
		TypeSpec.Builder supportClassBuilder = TypeSpec.classBuilder(supportClassName)
			...
			.addType(tableClassBuilder.build());

		// 添加外部类的表单例
		FieldSpec tableInstance = FieldSpec
			.builder(tableClassName, tableFieldName, Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)
		...
		for (VariableElement field : fields) {
			...
			FieldSpec shortcutField = FieldSpec
				.builder(ParameterizedTypeName.get(sqlColumnClass, fieldType), fieldName, Modifier.PUBLIC,
						Modifier.STATIC, Modifier.FINAL)
				.initializer("$L.$L", tableFieldName, fieldName)
				.build();
		...
		try {
			...
			processingEnv.getMessager()
				.printMessage(Diagnostic.Kind.ERROR, "Failed to generate " + supportClassName + ": " + e.getMessage());
		}
	}
	...
	private String inferJdbcType(TypeName typeName) {
		...
		if (name.contains("Long"))
			return "BIGINT";
		if (name.contains("Date") || name.contains("LocalDateTime"))
			return "TIMESTAMP";
		if (name.contains("LocalDate"))
			return "DATE";
		if (name.contains("LocalTime"))
			return "TIME";
		if (name.contains("BigDecimal"))
			return "DECIMAL";
